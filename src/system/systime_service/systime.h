...
public:
  ...
  // ===== Compatibility API (Old names) =====
  inline time_t get_time() { return now(); }
  inline void set_time(time_t t) { set(t); }
  inline void set_time(const uint8_t *buf) { set(buf); }
  inline void reserve_to_memory() { saveToRTC(); }
