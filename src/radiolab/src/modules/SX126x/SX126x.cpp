...
int16_t SX126x::setPreambleLength(size_t preambleLength)
{
  ...
  {
    this->preambleLengthFSK = preambleLength;
    this->preambleDetLength = preambleLength >= 32 ? RADIOLIB_SX126X_GFSK_PREAMBLE_DETECT_32 : preambleLength >= 24 ? RADIOLIB_SX126X_GFSK_PREAMBLE_DETECT_24
...
int16_t SX126x::modSetup(float tcxoVoltage, bool useRegulatorLDO, uint8_t modem)
{
  ...
  Serial.print(" SX126x Irq pin:");
  Serial.println(this->mod->getIrq());
  Serial.print(" SX126x Gpio pin:");
  Serial.println(this->mod->getGpio());
