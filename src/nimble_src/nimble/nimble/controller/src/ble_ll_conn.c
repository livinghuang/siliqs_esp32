...
#include <string.h>
#include <assert.h>
#include "nimble_src/nimble/porting/nimble/include/syscfg/syscfg.h"
#include "nimble_src/nimble/porting/nimble/include/os/os.h"
#include "nimble_src/nimble/porting/nimble/include/os/os_cputime.h"
#include "nimble_src/nimble/nimble/include/nimble/ble.h"
#include "nimble_src/nimble/nimble/include/nimble/nimble_opt.h"
#include "nimble_src/nimble/nimble/include/nimble/hci_common.h"
#include "nimble_src/nimble/nimble/include/nimble/ble_hci_trans.h"
 ...
 */
int ble_ll_conn_chk_phy_upd_start(struct ble_ll_conn_sm *csm)
{
    int rc;
...
ble_ll_conn_tx_pdu(struct ble_ll_conn_sm *connsm)
{
    ...
    if (!connsm->cur_tx_pdu && !CONN_F_EMPTY_PDU_TXD(connsm))
    {
             ...
             CONN_IS_SLAVE(connsm)))
        {
             ...
             */
            if (nextpkthdr && !ble_ll_ctrl_enc_allowed_pdu_tx(nextpkthdr) && ((connsm->conn_role == BLE_LL_CONN_ROLE_MASTER) || !ble_ll_ctrl_is_start_enc_rsp(m)))
            {
                nextpkthdr = NULL;
            }
        }
    ...
    else
    {
        ...
        if (connsm->cur_tx_pdu)
        {
            ...
            if (connsm->enc_data.enc_state > CONN_ENC_S_ENCRYPTED)
            {
                ...
                if (nextpkthdr && !ble_ll_ctrl_enc_allowed_pdu_tx(nextpkthdr) && ((connsm->conn_role == BLE_LL_CONN_ROLE_MASTER) || !ble_ll_ctrl_is_start_enc_rsp(connsm->cur_tx_pdu)))
                {
                    nextpkthdr = NULL;
                }
            }
...
void ble_ll_conn_rd_features_timer_cb(struct ble_npl_event *ev)
{
    struct ble_ll_conn_sm *connsm;
...
#else
        connsm->ce_end_time = connsm->anchor_point +
                              (MYNEWT_VAL(BLE_LL_CONN_INIT_SLOTS) * BLE_LL_SCHED_32KHZ_TICKS_PER_SLOT) + os_cputime_usecs_to_ticks(connsm->slave_cur_tx_win_usecs) + 1;
#endif
 ...
 */
int ble_ll_conn_connect_ind_send(struct ble_ll_conn_sm *connsm, uint8_t end_trans)
{
    int rc;
 ...
 */
int ble_ll_conn_slave_start(uint8_t *rxbuf, uint8_t pat, struct ble_mbuf_hdr *rxhdr,
                            bool force_csa2)
{
    int rc;
    uint32_t temp;
