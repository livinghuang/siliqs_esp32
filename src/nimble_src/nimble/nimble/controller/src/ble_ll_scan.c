...
struct ble_ll_scan_dup_entry
{
    ...
    uint16_t adi;
#endif
    TAILQ_ENTRY(ble_ll_scan_dup_entry)
    link;
};
...
void ble_ll_scan_aux_data_unref(struct ble_ll_aux_data *aux_data)
{
    os_sr_t sr;
 ...
 * @param rxbuf
 */
void ble_ll_scan_rx_pkt_in(uint8_t ptype, struct os_mbuf *om, struct ble_mbuf_hdr *hdr)
{
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
    struct ble_mbuf_hdr_rxinfo *rxinfo = &hdr->rxinfo;
    struct ble_ll_aux_data *aux_data = rxinfo->user_data;
