 ...
 */
static int
get_nvs_matching_index(void *nvs_val, void *db_list, int db_num, size_t item_size)
{
...
ble_store_nvs_write(int obj_type, const union ble_store_value *val)
{
    ...
    if (obj_type == BLE_STORE_OBJ_TYPE_CCCD)
    {
        return ble_nvs_write_key_value(key_string, &val->cccd, sizeof(struct ble_store_value_cccd));
    }
    else
    {
        return ble_nvs_write_key_value(key_string, &val->sec, sizeof(struct ble_store_value_sec));
    }
}
...
ble_store_nvs_peer_records(int obj_type, const struct ble_hs_dev_records *p_dev_rec)
{
    ...
    return ble_nvs_write_key_value(key_string, p_dev_rec, sizeof(struct ble_hs_dev_records));
}
#endif
