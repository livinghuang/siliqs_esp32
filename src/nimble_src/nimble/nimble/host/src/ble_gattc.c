...
#include "nimble_src/nimble/nimble/host/include/host/ble_gap.h"
...
static os_membuf_t ble_gattc_proc_mem[OS_MEMPOOL_SIZE(MYNEWT_VAL(BLE_GATT_MAX_PROCS),
...
/* Statistics. */
STATS_SECT_DECL(ble_gattc_stats)
ble_gattc_stats;
 ...
 */
void ble_gattc_rx_find_type_value_hinfo(uint16_t conn_handle,
                                        struct ble_att_find_type_value_hinfo *hinfo)
{
 ...
 */
void ble_gattc_rx_read_group_type_adata(uint16_t conn_handle,
                                        struct ble_att_read_group_type_adata *adata)
{
...
int ble_gattc_init(void)
{
    ...
    rc = stats_init_and_reg(
        STATS_HDR(ble_gattc_stats), STATS_SIZE_INIT_PARMS(ble_gattc_stats, STATS_SIZE_32), STATS_NAME_INIT_PARMS(ble_gattc_stats), "ble_gattc");
    if (rc != 0)
    {
        return BLE_HS_EOS;
    }
